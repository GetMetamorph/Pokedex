...
export class PokemonsDto {
  createPokemon = (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!req.body.description) throw new HttpException(400, 'Description field required');
      if (!req.body.height) throw new HttpException(400, 'Height field required');
      if (!req.body.Weight) throw new HttpException(400, 'Weight field required');
  ...
  updatePokemon = (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (req.body.type1) {
        if (req.body.type1.trim().length === 0) throw new HttpException(400, "Type1 can't be empty");
      }

      if (req.body.type2) {
        if (req.body.type2.trim().length === 0) throw new HttpException(400, "Type2 can't be empty");
      }

      if (req.body.description) {
        if (req.body.description.trim().length === 0) throw new HttpException(400, "Description can't be empty");
      }

      if (req.body.height) {
        if (req.body.height.trim().length === 0) throw new HttpException(400, "Height can't be empty");
      }

      if (req.body.weight) {
        if (req.body.weight.trim().length === 0) throw new HttpException(400, "Weight can't be empty");
      }
